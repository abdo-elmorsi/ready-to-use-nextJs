...
export const getAddress = async (_SerialNumber, _div = null, _updateList = true, trial = 0) => {
    let _USER_VEHICLES = JSON.parse(localStorage.getItem(encryptName('uservehs'))) ?? [];
    let pubLocInfo = _USER_VEHICLES.find(x => x.SerialNumber == _SerialNumber);
    try {
        let getJson = Object.assign({}, {
            Address: data,
